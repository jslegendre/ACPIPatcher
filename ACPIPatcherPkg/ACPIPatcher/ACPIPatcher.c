...
VOID *
NewBufferFromData(VOID * Source, UINT8 length)
{
...
CHAR16 * 
GetPath(CHAR16 * FullName) 
{
	int Index = StrLen(FullName);

 	while(Index != 0 && StrnCmp(&FullName[Index], L"\\", 1)) {
 		Index--;
 	}
 	...
 	Index = (Index * 2) + 2;
 	CHAR16 * path = (CHAR16 *)NewBufferFromData(FullName, Index);
 	path[Index - 2] = 0x0;
 	return path;
...
GetCurrentPath() 
{
  ...
  //Arg0 = ShellParameters->Argv[0];
  //Print(L"Generating path from: %s\n", Arg0)
...
EFI_STATUS
OpenACPIFolder(EFI_SHELL_FILE_INFO ** ACPIFolderHead) 
{
...
UINT64
ReadAMLFileToBuffer(const CHAR16 * AMLFile) {
